...
export default class BaseRepository<T extends Document> implements Read<T>, Write<T> {
  ...
  async find(cond: any, fields: any, options: any, sortOptions?: any, populate: boolean = true, maxTimeMS?: number): Promise<T[]> {
    return new Promise<T[]>((resolve, reject) => {
  ...
  async findOne(cond: any, fields: any, options: any): Promise<T> {
    return new Promise<T>((resolve, reject) => {
      this._model.findOne(cond, fields, options).populate('revisions').lean({ virtuals: true }).exec((err: any, res: T) => {
        if (err) {
          reject(err);
        } else {
          resolve(<T>res);
  ...
  async paginationBrandWithCondition(cond: any, projection: any, option: any): Promise<T> {
    return new Promise<T>((resolve, reject) => {
      this._model.find({ $and: [{ deleted: false }, cond] }, {}, option, (err: any, res: any) => {
        if (err) {
          reject(err);
  ...
  async hardDelete(cond: any): Promise<T> {
    return new Promise<T>((resolve, reject) => {
  ...
  conditionAmazonBrand() {
    return { 'marketplaces.amazon_com': true };
  }
  ...
  async aggregateAllowDisk(operationList: any): Promise<any> {
    return new Promise<any>((resolve, reject) => {
      this._model.aggregate(operationList).allowDiskUse(true)
